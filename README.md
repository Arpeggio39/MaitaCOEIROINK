...
自動モーションは発声の強調に合わせた首・体・肩・腕の動きと、ゆっくりした重心移動を組み合わせます。大きな動きには予備動作と遅れをつけ、休止中は動きを抑えます。モデルに存在する可動範囲内で動作します。

「言葉に合わせて表情を変える」で、ありがとう・嬉しい・びっくり・残念・なぜ等の単語から表情を選び、滑らかに変化させます。簡単な否定表現は除外します。切り替え時刻は文章内の位置と音声の発声区間から推定するため、厳密な単語単位の同期や文脈理解ではありません。
