...
編集ソフトのクロマキーで緑色を抜いて合成してください。保存形式はMP4固定です。動画のサイズとアスペクト比は、口パク・モーション・物理演算を反映した全フレームの描画範囲を調べ、小さな余白を加えて自動決定します。動画内では画角を固定します。Live2D描画と領域の集約はWebGLのGPU処理を利用し、NVIDIA環境では利用可能なNVENCと空きVRAM・メモリに応じた並列出力を利用します。
口は音量に合わせて滑らかに開閉します。母音別の口形認識は行いません。
10分以内の音声に対応します。全フレームの領域確認後に動画を出力し、所要時間は音声の長さ・動き・PC性能によって変わります。進行状況は共通のメーターに表示します。
口パク用の音量解析には同梱のC製WebAssembly SIMDを使用します。利用できない場合はJavaScript処理へ切り替わります。
...
同じ音声・文章では同じ動作の予定を使います。
既存モデルの可動範囲を使うため、モデルにない指差しや手振りのポーズは生成しません。

### 収録したモーションを使用する

動画付き出力をONにすると、書き出しモーダルで同じマイタモデル用の `.motion3.json` を選択できます（10MBまで）。体・目・眉・口の形などを収録モーションから再生し、口の開閉だけを音声に合わせます。ファイル未選択時は従来の自動モーションを使います。

「音声より短い場合は繰り返す」をOFFにすると、モーション終了後は最後の姿勢を保持します。区切りごとの出力でも、全文の音声時刻を基準にモーションを継続します。元ファイルは変更しません。モーションカーブはそのままの強さで適用し、ファイル内のフェード指定・ユーザーデータイベントは使用しません。

「タイミングを合わせる」を押すと、合成したナレーションとモーションを同じ時間軸に表示します。モーションの長方形を右へドラッグすると遅れて開始し、左へドラッグすると途中から開始します。秒数入力・矢印キーでの調整、音声の試聴もできます。「適用」で書き出しに反映します。開始前は最初の姿勢を保持し、口パクはナレーションに追従します。

テスト用の大胆な8秒モーションは `node scripts/generate-bold-motion.cjs` で `local-output/maita-bold.motion3.json` に生成できます。

### 文章・音声に合わせた自動演技

自動モーションは発声の強調に合わせた身振りと、感謝・謝罪・喜び・驚き・疑問・思案・相づち・挨拶・説明などの言葉を手掛かりにした演技を組み合わせます。感謝では軽くお辞儀し、疑問では首をかしげるなど、顔と体で同じ演技の予定を使います。反応の最中は別のうなずきを重ねず、左右の肩・腕・手・指には異なる動きと遅れをつけます。

視線を変えるときは目が先に動き、頭が追従します。話している間は胴体ごと前後左右に大きく姿勢を変えて保ち、重心移動には頭が少し遅れてついてきます。短い台詞でも動き始め、休止中は身振りや重心移動を抑えて呼吸とまばたきを続けます。動作は60 Hzで事前計算し、再生・書き出しで同じ予定を参照します。モデルに存在する可動範囲内で動作します。

「言葉に合わせて表情を変える」で、口形・笑顔の目・まぶた・眉を滑らかに変化させます。表情同士が重なる場合は顔全体を混ぜ、まばたきを残します。OFFでは言葉に合わせた顔の表情変更を止め、身振りは継続します。

全文出力と動画プレビューでは、合成した区切りごとの実際の音声の長さを使って演技を配置します。その区切り内の言葉の時刻は文字数と発声区間からの推定です。簡単な否定表現は除外しますが、皮肉や引用を含む文脈理解・厳密な単語単位の同期ではありません。外部AIサービスや追加のモデルは不要です。

モーションファイルを選択した場合、体・視線・表情・口の形などは収録データを優先し、口の開閉だけを音声で上書きします。自動モーションと単語による表情変更は適用しません。
